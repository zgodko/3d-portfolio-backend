...
int main() {
    ...
    svr.Get("/api/models", [&modelsData](const httplib::Request &req,
        ...
        int totalPages = (total + limit - 1) / limit; // Округление вверх

        // Вычисляем диапазон для текущей страницы
        int startIndex = (page - 1) * limit;
        int endIndex = std::min(startIndex + limit, total);

        // Формируем список моделей для текущей страницы
    ...
    svr.Get(R"(/api/models/([^/]+))",
            [&modelsData](const httplib::Request &req, httplib::Response &res) {
